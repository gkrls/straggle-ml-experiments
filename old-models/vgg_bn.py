...
def train(args):
    ...
    optimizer = optim.SGD(
        ...
        weight_decay=args.weight_decay, nesterov=False, foreach=True
    )
...
if args.warmup_epochs and args.warmup_epochs > 0:
    ...
    if straggle_sim.attach(model):
        print(f"Straggle sim initialized with {straggle_sim}")
    else:
        print("Straggle sim inactive")
...
def setup_ddp(args):
    ...
    if args.device == 'cuda' and backend == 'gloo':
        backend = 'nccl'  # prefer NCCL on GPU

    dist.init_process_group(backend=backend, init_method="env://", rank=args.rank, world_size=args.world_size, timeout=datetime.timedelta(seconds=60))

    print(f"[DDP] backend={backend} world_size={args.world_size} master={args.master_addr}:{args.master_port} iface={args.iface} local_rank={args.local_rank}", flush=True)
...
def main():
    ...
    parser.add_argument('--json', type=str, default='vgg_bn.json')
    ...
    # Seeding (only if deterministic requested)
    if args.deterministic:
