...
PATTERN = {
    ...
    2: lambda args: torch.ones(args.size, dtype=args.dtype, device=torch.device(args.device)) * (args.rank + 1),  # rank+1,rank+1,rank+1...
    3: lambda args: torch.arange(args.size, device=torch.device(args.device)).remainder(5).add(1).to(args.dtype), # 1,2,3,4,5,1,2,3...
}
...
PATTERN_OUT = {
    ...
    # 2: lambda args: torch.full(args.size,( (args.world_size + 1) / 2 if args.avg else args.world_size * (args.world_size + 1) / 2),
    # dtype=args.dtype,
...
def init(args):
    ...
    if args.backend.startswith("dpa"):
        ...
        print(f"[Rank {args.rank}] Using DPA with {'dpdk' if 'dpdk' in args.backend  else 'socket'} backend") #
