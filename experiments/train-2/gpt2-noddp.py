#!/usr/bin/env python3
"""
GPT-2 (124M) Vanilla Distributed trainer (no DDP) on local Parquet OpenWebText.
"""
...
def sync_gradients(model, world_size):
    """Manually allreduce gradients across all ranks"""
...
def broadcast_parameters(model, src=0):
    """Broadcast model parameters from rank 0 to ensure all ranks start with same weights"""
    for param in model.parameters():
...
def train_one_epoch(model, dataloader, optimizer, device, scaler, args,
    ...
    while True:
        ...
        if is_last_micro:
            ...
            # Manual gradient synchronization (replaces DDP's automatic sync)
...
def train(args):
    ...
    # Broadcast initial parameters from rank 0 to ensure all ranks start synchronized
    broadcast_parameters(model, src=0)

    # Wrap the model if DPA backend is requested (optional custom wrapper)
    if args.backend.startswith("dpa"):
        model = dpa.DDPWrapper(model, straggle=args.world_size, prescale=args.prescale)

    # Straggle sim (if using)
    ...
    if straggle.attach(model): 
        ...
        print(f"{straggle} created but inactive for rank {args.rank}")
    ...
    if args.rank == 0:
        ...
        print(f"  Mode: Vanilla Distributed (manual gradient sync)")
        print("=" * 60, flush=True)

    # param groups
    decay_params, nodecay_params = [], []
    for p in model.parameters():
        if p.requires_grad: (decay_params if p.ndim >= 2 else nodecay_params).append(p)
    ...
    if adj_micro_steps_per_epoch <= 0:  raise ValueError("micro_steps_per_epoch must be at least gradient_accumulation_steps.")
    if adj_micro_steps_per_epoch != orig_micro_steps_per_epoch:
        print(f"[{now()}][Note] micro_steps_per_epoch adjusted from {orig_micro_steps_per_epoch} "
              f"to {adj_micro_steps_per_epoch} to be a multiple of GA={GA}.", flush=True)
    args.micro_steps_per_epoch = adj_micro_steps_per_epoch
    ...
    # init JSON log
    if args.rank == 0:
        if args.json is None: args.json = "gpt2_vanilla_training.json"
        log = {
            "time": now(),
...
def setup_ddp(args):
    ...
    print(f"[{now()}][Distributed] backend={args.backend} world_size={args.world_size} "
          f"master={args.master_addr}:{args.master_port} iface={args.iface} local_rank={args.local_rank}", flush=True)


# ------------------------- main -------------------------
def main():
    parser = argparse.ArgumentParser(description='GPT-2 Vanilla Distributed Training on OpenWebText')
    ...
    parser.add_argument('--static_graph', action='store_true')  # Not used in vanilla distributed
    parser.add_argument('--workers', type=int, default=4)
    parser.add_argument('--seed', type=int, default=42)
    parser.add_argument('--json', type=str, default="gpt2_vanilla.json", help="Path to JSON run log")
    ...
    parser.add_argument("--bucket_cap_mb", type=int, default=None, help="Not used in vanilla distributed")
