...
class GPT2MemmapDataset(IterableDataset):
    """
    ...
    def __iter__(self):
        ...
        indices = rng.permutation(self.n_windows)
        ...
        for idx in indices:
            offset = self.start + idx * self.window
            tokens = self.data[offset:offset + self.window].astype(np.int64)
            yield torch.from_numpy(tokens)
...
def train(args, straggle, best_model_group):
    ...
    cfg = GPT2Config(vocab_size=vocab_size,n_positions=args.seq_len, n_ctx=args.seq_len,
                     n_embd=768, n_layer=12, n_head=12, n_inner=3072, activation_function="gelu",
                     resid_pdrop=0.1, attn_pdrop=0.1, embd_pdrop=0.1, layer_norm_epsilon=1e-5, initializer_range=0.02,
                     bos_token_id=tokenizer.bos_token_id, eos_token_id=tokenizer.eos_token_id, pad_token_id=tokenizer.eos_token_id,
    ...
    print(f"\n[{now()}] Dataset: Openwebtext (tokenized) at {data_root}")
    print(f"  Vocab size: {vocab_size:,} | Seq len: {args.seq_len}")
    print(f"  Train/Val samples: {train_ds.n_windows:,}/{val_ds.n_windows:,} of len {args.seq_len} + 1")
    print(f"[{now()}] Model: GPT-2 small | Params: {sum(p.numel() for p in model.parameters()):,}")
    ...
    if adj_micro_steps_per_epoch != orig_micro_steps_per_epoch:
        print(f"[{now()}][Warning] micro_steps_per_epoch adjusted from {orig_micro_steps_per_epoch} to {adj_micro_steps_per_epoch} to be a multiple of GA={GA}.", flush=True)
...
def setup_ddp(args):
    ...
    if args.backend.startswith("dpa"):
        ...
        if args.dpa_repin:
            os.sched_setaffinity(4, set(range(4, os.cpu_count() - dpa_backend.threads - 1)))
