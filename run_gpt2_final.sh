...
echo "[run_gpt2_final.sh] iface=$IFACE ip=$IP rank=$RANK world_size=$WORLD_SIZE master=${MASTER_ADDR}:${MASTER_PORT} backend=$BACKEND"
...
set -x
exec python -u $HOME/straggle-ml-experiments/models/gpt2_final.py \
  --rank "$RANK" \
