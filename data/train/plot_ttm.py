...
def aggregate_data(rank_datas, metric, agg, minival_metric=None):
    ...
    Time:   max across ranks per epoch (wall-clock = slowest rank).
    ...
    epoch_keys = sorted(rank_datas[rank_ids[0]]["epochs"], key=int)
    ...
    for ek in epoch_keys:
        times, vals, steps_list = [], [], []
        for rid in rank_ids:
            ep = rank_datas[rid]["epochs"][ek]
            times.append(ep["epoch_time"])
            vals.append(ep[metric])
            steps_list.append(ep["steps"])

        # Wall-clock = slowest rank
        agg_time = max(times)
        agg_steps = steps_list[0]
...
# def load_input(path_or_dir, rank_agg="rank0", exclude_ranks=None,
#                metric=None, minival_metric=None):
#     """Load from a file or directory with rank aggregation.

#     rank_agg: 'rank0', 'rank1', ..., 'avg', 'best'
#     """
...
def plot_tta(input_a, input_b, metric, labels=("BSP", "SA-INA"),
             ...
             rank_agg="rank0", exclude_ranks=None):
    if not input_a: "No input_a"
    if not input_b: "No input_b"
    ...
    da = load_input(input_a, rank_agg, exclude_ranks, metric, minival_metric)
    db = load_input(input_b, rank_agg, exclude_ranks, metric, minival_metric)
    ...
    print(f"% Rank aggregation: {rank_agg}")
    if exclude_ranks:
...
if __name__ == "__main__":
    ...
    p.add_argument("--rank", default="rank0", help="0, 1, ..., avg, best (default: rank0)")
