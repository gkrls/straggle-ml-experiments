...
echo "[run_vgg11_straggle.sh] iface=$IFACE ip=$IP rank=$RANK world_size=$WORLD_SIZE master=${MASTER_ADDR}:${MASTER_PORT} backend=$BACKEND"
...
set -x
exec python -u $HOME/straggle-ml-experiments/models/vgg.py \
  --rank "$RANK" \
  ...
  --straggle_verbose \
  --json $HOME/straggle-ml-experiments/models/vgg11_straggle.json \
  "$@"